class Dashboard::OrdersController < ApplicationController
  ...
  def show
    ...
    @order_items = @order.order_items.eager_load(product: {image_attachment: :blob}).where("store_id = ?", current_store.id)
  end
  ...
  def report
    ...
    @order_totals = @orders.joins(order_items: :product).where("store_id =?", current_store).group("orders.id").pluck(Arel.sql("orders.id, sum(order_items.price*quantity)")).to_h
  end
end
